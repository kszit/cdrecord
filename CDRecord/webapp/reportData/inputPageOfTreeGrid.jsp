...
<title><s:property value="dataReportModel.reportName"/></title>
...
<div data-options="region:'north',split:true" style="height:80px;">
	...
	<h2>
			...
			<s:elseif test="reportConfigModel.rttype==201"><!-- 年报表 -->
				<s:select cssClass="easyui-combobox" 
						name='dataReportModel.periods'
						id='dataReportModel_periods' 
				       	list="periodList"
				       	listKey="bindId"
				       	listValue="name"
	...
	<div style="position: absolute; right: 1px; top: 0px;" id="setting">
		 <div style="padding:5px;border:1px solid #ddd;">
			...
			<s:else><!-- 填报表 -->
				<a href="#" class="easyui-linkbutton" data-options="plain:true,iconCls:'icon-shenhe'" onclick="appearReport();return false;">提交审核</a>
				<a href="#" class="easyui-linkbutton" data-options="plain:true,iconCls:'icon-file-lock'" onclick="publishReport();return false;">直接上报</a>
				<a href="#" class="easyui-linkbutton" data-options="plain:true,iconCls:'icon-search'" onclick="selectReport();return false;">选择报表</a>
			</s:else>
		</div>
	</div>
...
<!-- excel数据文件上传 -->
    <div id='exceldataInputFromDiv' class="easyui-dialog" title="d" data-options="iconCls:'icon-save',modal:true,closed:true" 
		...
		<form target="hidden_frame" id="exceldataInputFrom"
			...
			<s:hidden name="dataReportModel.periods" id='upForm_dataReportModel_periods'></s:hidden>
			<s:hidden name="dataReportModel.year" id='upForm_dataReportModel_year'></s:hidden>
...
var toolbar = [{
		...
		handler:function(){exportExcelModel();}
	},{
		text:'导入数据',
		iconCls:'icon-import',
		handler:function(){importExcelData()}
	},'-',{
		text:'合并子项',
